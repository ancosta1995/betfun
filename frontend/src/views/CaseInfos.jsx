...
const useStyles = makeStyles(theme => ({
  root: {
    padding: "32px 100px", // Default padding
    minHeight: "calc(120vh - 7rem)",
    width: "100%",
    marginTop: "auto",
    color: "#fff",
    fontFamily: "Rubik",
    ...
    [theme.breakpoints.down("sm")]: {
      padding: "32px", // Adjusted padding for screens with max-width of 800px
    },
  },
  topBar: {
    backgroundColor: "rgb(16, 17, 35)",
  ...
  interactionContainer: {
    ...
    borderBottomLeftRadius: "0.5rem", // Modified property name
    borderBottomRightRadius: "0.5rem", // Modified property name
  },
  ...
  demoButton: {
    ...
    display: "flex",
    position: "relative",
    color: "#9E9FBD", // Modified property name
    cursor: "pointer",
    padding: "0.5rem",
    userSelect: "none", // Modified property name
    borderRadius: "0.25em", // Modified property name
    backgroundColor: "hsla(0,0%,100%,.05)", // Modified property name
    transitionDuration: "300ms", // Modified property name
    "&:hover": {
      opacity: 0.75 // Adjusted hover opacity
    }
  },
  ...
  winItem: {
    position: "relative",
    zIndex: 100000,
    animation: "$float 2.5s ease-in-out infinite", // Adding animation here
  },
  "@keyframes float": {
    "0%": {
...
for (const box of w) {
    ...
    box.style.transition = 'all .25s ease-out'; // Added transition
    box.style.transform = 'scale(1.2)';
    // box.style.animation = 'float 2.5s ease-in-out infinite';
    box.style.padding = '.25rem';
    // box.style.filter = 'drop-shadow(-6px 6px 0 rgba(0, 0, 0, .25))';
    box.style.opacity = '1';
    box.style.background = '#101123';
    // box.style.border = 'solid rgb(0 125 255)';
    box.style.borderRadius = '.5rem';
    box.style.borderWidth = '2px 2px 4px';
    // box.style.filter = 'drop-shadow(0 0 10px rgb(0 125 255))';
    ...
    imgElement.src = 'https://i.ibb.co/LxpnfZP/dl-2a39d38a.webp';
    imgElement.alt = 'Case Cost';
    imgElement.width = '14';
    imgElement.height = '14';
    imgElement.style.marginLeft = '5px'; // Adjust margin as needed
    imgElement.style.marginBottom = '-2px'; // Adjust margin as needed

    // Create a span element for the number
    const numberSpan = document.createElement('span');
    numberSpan.textContent = `${number}`;

    // Append the number span and the image to the numberElement
  ...
  const getFilterForItem = (item) => {  
    ...
    if (item.color === "blue") {
      ...
      filter = "drop-shadow(rgba(0, 0, 0, 0.3) -6px 6px 0px)"; // purple
    } else if (item.color === "pink") {
      filter = "drop-shadow(rgba(0, 0, 0, 0.3) -6px 6px 0px)"; // pink
    } else if (item.color === "gold") {
      filter = "drop-shadow(rgba(0, 0, 0, 0.3) -6px 6px 0px)"; // gold
    } else if (item.color === "orange") {
      filter = "drop-shadow(rgba(0, 0, 0, 0.3) -6px 6px 0px)"; // orange
    } else {
      filter = "drop-shadow(rgba(0, 0, 0, 0.3) -6px 6px 0px)"; // red
    }
    return filter;
  };

  const copyLinkAction = () => {
    const caseLink = `https://betfun.gg/cases/${caseSlug}`;
  ...
  const getBorderForItem = (item) => {
    let border = "";
    if (item.color === "blue") {
      border = "0px 0px 10px 0px rgba(0, 0, 0, 0.5)"; // gold
    } else if (item.color === "purple") {
      border = "0px 0px 10px 0px rgba(0, 0, 0, 0.5)"; // gold
    } else if (item.color === "pink") {
      border = "0px 0px 10px 0px rgba(0, 0, 0, 0.5)"; // gold
        ...
        allBoxes.push(
          <div className={classes.lootBoxContainer} style={{background: background, borderBottom: "3px solid " + border,}}>
            ...
            <span className={classes.price}>{parseCommasToThousands(item?.price)}</span>
                  <img src="https://i.ibb.co/LxpnfZP/dl-2a39d38a.webp" alt="Case Cost" width="13" height="13" style={{ marginLeft: '5px' }} />
    ...
    <div className={classes.root}> 
      ...
      <div className={classes.container}>
        ...
        <div className={classes.interactionContainer}>
          ...
          <div className={`${demo ? classes.caseButtonDisabled : "" } `} onClick={() => openCase()}>
...
<div className={classes.casePriceSpan}>

    <span className={classes.casePriceSpanText}>{parseCommasToThousands(caseData?.price)}</span>
    <img src="https://i.ibb.co/LxpnfZP/dl-2a39d38a.webp" alt="Case Cost" width="14" height="14" style={{ marginBottom: '-2px', marginLeft: '5px' }} />
      ...
      <br></br>
      <span className={classes.lootTableTitle}>Items in case</span>
      <br></br>
      <br></br>
