...
const MarketPage = ({ user, open, handleClose}) => {
  ...
  return (
    !user ? (
      ...
      <div className={classes.root}>        <div style={{flex: "1 1 0%",overflow: "hidden"}}>
      <div className={classes.topBar}>
        ...
        <div className={classes.topRight}>
          ...
          onClick={() => switchSortType("mm2")}>GT</Button>
          <Button className={`${classes.button} ${
            sortType === "adptme" ? classes.selected : ""
          }`} style={{ fontSize: "0.7rem" }}
          onClick={() => switchSortType("adptme")}>BETA</Button>
        </div>
      </div>
      <div className={classes.itemsContainer}>
        ...
        <div style={{paddingLeft: "1.5rem",paddingRight: "1.5rem"}}>
          ...
          {items
.filter(item => item.display_name && item.display_name.toLowerCase().includes(searchInputState.toLowerCase()))
.sort((a, b) => sortHigh ? a.priceUSD - b.priceUSD : b.priceUSD - a.priceUSD)
.map((item, index) => {
              return ( 
                <div 
