...
const Slots = ({ user, logout }) => {
  ...
  useEffect(() => {
    ...
    // For this example, we are using a hardcoded JSON object
    const fetchData = async () => {
      // Simulate fetching data from an API or file
      const response = await fetch("https://raw.githubusercontent.com/blancos13/games/refs/heads/main/live_baccarat_games.json"); // Adjust path as necessary
      const data = await response.json();
      const gamesArray = Object.values(data); // Convert the object to an array
      
      setGameData(gamesArray);
      setFilteredGameData(gamesArray);

      const uniqueProviders = [...new Set(gamesArray.map(game => game.category))];
      setProviders(uniqueProviders);

      const counts = uniqueProviders.reduce((acc, provider) => {
        acc[provider] = gamesArray.filter(game => game.category === provider).length;
        return acc;
      }, {});
  ...
  const debouncedSearch = useCallback(
    debounce((query, selectedProviders, gameData) => {
      ...
      if (selectedProviders.length > 0) {
        filteredData = filteredData.filter(game => selectedProviders.includes(game.category));
      }

      // Sort the data based on the selected sorting option
      switch (sortOption) {
        case 'Newest release':
          filteredData.sort((a, b) => new Date(b.ts * 1000) - new Date(a.ts * 1000));
          break;
          ...
          <div className={classes.container}>
            <div className={classes.titleContainer}>
              <Buttons
                ...
                onClick={() => history.goBack()} // Navigate back on button click
              >
                Back
              </Buttons>
            ...
            <div className={classes.gridContainer}>
              {filteredGameData.slice(0, displayedImages).map((game, index) => (
                <Link to={`/slots/${game.id_hash}`} key={index} className={classes.gridItem}>
                  <GameImage game={game} />
            ...
            {displayedImages < filteredGameData.length && (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                whileHover={{ scale: 1.05 }} // Slight scale on hover for better UX
                whileTap={{ scale: 0.95 }} // Slight scale down on click for better feedback
