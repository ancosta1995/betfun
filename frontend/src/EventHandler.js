...
import chingSound from "./assets/balance-change.webm";

const chingAudio = new Audio(chingSound);
chingAudio.volume = 0.25;
...
const EventHandler = withRouter(({ changeWallet, history }) => {
  ...
  useEffect(() => {
    ...
    const updateWallet = ({ amount, currency }) => {
      changeWallet({ amount, currency });
      if (amount > 0) {
        playSound(chingAudio);
      }
    };
