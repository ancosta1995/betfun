...
const CupAnimation = ({ 
  ...
  const drawLine = (ctx, canvas, XScale, YScale, offset_bottom, offset_left, current_time, stage) => {
    ...
    // Safety check - if no points, return early
    if (points.length === 0) {
      ctx.restore();
      return;
    }
    ...
    // Handle last points if we have enough points
    if (points.length > 2) {
    ...
    // Handle last points for the line
    if (points.length > 2) {
        const last = points.length - 1;
        ctx.quadraticCurveTo(points[last - 1].x, points[last - 1].y, points[last].x, points[last].y);
    } else if (points.length === 2) {
    ...
    // Draw crash effects
    if (stage === 'crashed' && !crashSettings.lastCrashPoint && points.length > 0) {
    ...
    // Draw progress effects
    if (stage === 'progress' && points.length > 0) {
      const lastPoint = points[points.length - 1];
  ...
  useEffect(() => {
    ...
    animateRef.current = () => {
      ...
      switch (gameState) {
        ...
        default:
          stage = 'starting';
          return; // Don't animate in starting state
      }
      ...
      if (gameState === GAME_STATES.InProgress && startedAt) {
      ...
      <div className={classes.meta} style={{ 
        ...
        {!initialLoading && (gameState === GAME_STATES.InProgress || gameState === GAME_STATES.Over) && ownBet && (
          <div className={`${classes.profit} ${ownBet.status === BET_STATES.CashedOut ? 'cashed' : ''}`}>
            +{((ownBet.betAmount || 0) * payout).toFixed(2)}
            <span style={{ 
