...
const numLevels = parseInt(config.games.vip.numLevels);
const minWager = parseFloat(config.games.vip.minWager);
const maxWager = parseFloat(config.games.vip.maxWager);
const rakeback = parseFloat(config.games.vip.rakeback);
...
function generateVIPLevels(numLevels, minWager, maxWager, rakeback, levelNames, levelColors) {
  ...
  for (let i = 0; i < numLevels; i++) {
    const level = {
      name: (i + 1).toString(),
      wagerNeeded: (minWager + (maxWager - minWager) * Math.pow(i / numLevels, 5)).toFixed(2),
...
async function checkAndApplyRakeback(userId, houseRake) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const currentLevel = await getVipLevelFromWager(user.wager || 0);
      
      // Calculate rakeback amount, ensuring it's a valid number
      const rakebackAmount = parseFloat((houseRake * (parseFloat(currentLevel.rakebackPercentage) / 1000)).toFixed(8));
      
      if (isNaN(rakebackAmount)) {
        console.error('Invalid rakeback calculation:', { houseRake, rakebackPercentage: currentLevel.rakebackPercentage });
        return resolve();
      }
      ...
      await User.updateOne(
        { _id: user.id },
        {
          $inc: { rakebackBalance: rakebackAmount },
        }
      );
